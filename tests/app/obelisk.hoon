...
++  test-sys-sys-databases
  ...
  =.  run  +(run)
  =^  mov4  agent  
    (~(on-poke agent (bowl [run ~2000.1.4])) %obelisk-action !>([%tape-create-db "CREATE DATABASE db2"]))
  =.  run  +(run)
  =^  mov5  agent  
    (~(on-poke agent (bowl [run ~2000.1.5])) %obelisk-action !>([%tape %db1 "CREATE TABLE db2..my-table (col1 @t) PRIMARY KEY (col1)"]))
  =.  run  +(run)
  =^  mov6  agent  
    (~(on-poke agent (bowl [run ~2000.1.5])) %obelisk-action !>([%tape %db1 "FROM sys.sys.databases SELECT *"]))
  %+  expect-eq                         :: expected results
    !>  expected
    !>  ->+>+>.mov6
++  test-sys-tables
  ...
  =.  run  +(run)
  =^  mov5  agent  
    (~(on-poke agent (bowl [run ~2000.1.5])) %obelisk-action !>([%tape %db1 "CREATE NAMESPACE ref"]))
  =.  run  +(run)
  =^  mov6  agent  
    (~(on-poke agent (bowl [run ~2000.1.6])) %obelisk-action !>([%tape %db1 "CREATE TABLE db1..my-table-4 (col1 @p, col2 @t, col3 @ud) PRIMARY KEY (col1, col3)"]))
  =.  run  +(run)
  =^  mov7  agent  
    (~(on-poke agent (bowl [run ~2000.1.7])) %obelisk-action !>([%tape %db1 "CREATE TABLE db1.ref.my-table-4 (col1 @p, col2 @t, col3 @ud) PRIMARY KEY (col1, col3)"]))
  =.  run  +(run)
  =^  mov8  agent  
    (~(on-poke agent (bowl [run ~2000.1.8])) %obelisk-action !>([%tape %db1 "FROM sys.tables SELECT *"]))
  %+  expect-eq                         :: expected results
    !>  expected
    !>  ->+>+>.mov8
++  test-sys-columns
  =|  run=@ud
  =/  col-row  ~[[%namespace %tas] [%name %tas] [%col-ordinal %ud] [%col-name %tas] [%col-type %tas]]
  ...
  =/  expected  ~[%results ~[%result-set ~.db1.sys.columns col-row row1 row2 row3 row4 row5 row6 row7 row8 row9 row10]]
  =/  cmd
    [%drop-table table=[%qualified-object ship=~ database='db1' namespace='dbo' name='my-table'] %.y]
  =^  mov1  agent  
    (~(on-poke agent (bowl [run ~2000.1.1])) %obelisk-action !>([%tape-create-db "CREATE DATABASE db1"]))
  =.  run  +(run)
  =^  mov2  agent  
    (~(on-poke agent (bowl [run ~2000.1.2])) %obelisk-action !>([%tape %db1 "CREATE NAMESPACE db1.ref"]))
  =.  run  +(run)
  =^  mov3  agent  
    (~(on-poke agent (bowl [run ~2000.1.3])) %obelisk-action !>([%tape %db1 "CREATE TABLE db1.ref.my-table (col1 @t, col2 @t) PRIMARY KEY (col1, col2 DESC)"]))
  =.  run  +(run)
  =^  mov4  agent  
    (~(on-poke agent (bowl [run ~2000.1.4])) %obelisk-action !>([%tape %db1 "CREATE TABLE db1..my-table-2 (col1 @p, col2 @t) PRIMARY KEY (col1 desc, col2); CREATE TABLE db1..my-table-3 (col1 @p, col2 @t, col3 @ud) PRIMARY KEY (col1)"]))
  =.  run  +(run)
  =^  mov5  agent  
    (~(on-poke agent (bowl [run ~2000.1.5])) %obelisk-action !>([%tape %db1 "CREATE TABLE db1..my-table-4 (col1 @p, col2 @t, col3 @ud) PRIMARY KEY (col1, col3)"]))
  =.  run  +(run)
  =^  mov6  agent  
    (~(on-poke agent (bowl [run ~2000.1.6])) %obelisk-action !>([%tape %db1 "FROM sys.columns SELECT *"]))
  %+  expect-eq                         :: expected results
    !>  expected
    !>  ->+>+>.mov6
++  test-sys-log
  ...
  =.  run  +(run)
  =^  mov7  agent  
    (~(on-poke agent (bowl [run ~2000.1.8])) %obelisk-action !>([%tape %db1 "FROM sys.sys-log SELECT *"]))
  %+  expect-eq                         :: expected results
    !>  expected
    !>  ->+>+>.mov7
++  test-data-log
